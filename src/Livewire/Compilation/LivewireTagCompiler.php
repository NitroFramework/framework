...
namespace Nitro\Livewire\Compilation;
...
class LivewireTagCompiler
{
    ...
    protected static function compilePaired(string $template): string
    {
        ...
        return preg_replace_callback($pattern, static function (array $matches): string {
            $name = $matches[1];
            if ($name === 'slot') {
                return $matches[0]; // handled inside its parent, not on its own
            }

            $params = self::attributesToArray($matches[2] ?? '');
            [$named, $default] = self::extractSlots($matches[3] ?? '');
            ...
            foreach ($named as $slotName => $content) {
                ...
                $php .= "<?php ob_start(); ?>" . $content
                    . "<?php \$__slots['{$key}'] = new \\Nitro\\Livewire\\Support\\Slot(ob_get_clean()); ?>";
            }
            $php .= "<?php ob_start(); ?>" . $default
                . "<?php \$__slots['default'] = new \\Nitro\\Livewire\\Support\\Slot(trim(ob_get_clean())); ?>";
    ...
    protected static function extractSlots(string $inner): array
    {
        ...
        $default = preg_replace_callback($pattern, static function (array $matches) use (&$named): string {
            $slotName = ($matches[1] ?? '') !== '' ? $matches[1] : ($matches[2] ?? '');
            $named[$slotName] = $matches[3];
