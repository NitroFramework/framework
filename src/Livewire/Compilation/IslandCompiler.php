...
class IslandCompiler
{
    ...
    protected static function compileBlock(array $matches): string
    {
        [$name, $options] = self::parseArguments($matches[1]);
        [$body, $placeholder] = self::extractPlaceholder($matches[2]);
    ...
    {
        if (preg_match('/(.*?)@placeholder(.*?)@endplaceholder(.*)/s', $inner, $matches)) {
